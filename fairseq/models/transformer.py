...
class TransformerS2EncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask, bert_encoder_out, bert_encoder_padding_mask):
        ...
        x = self.maybe_layer_norm(self.self_attn_layer_norm, x, before=True)#a
        x1, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
        x1 = F.dropout(x1, p=self.dropout, training=self.training)
        x = residual + x1
        x = self.maybe_layer_norm(self.self_attn_layer_norm, x, after=True)#a
        
        x2 = self.maybe_layer_norm(self.self_attn_layer_norm, x2, before=True)#c
        x2, _ = self.bert_attn(query=x2, key=bert_encoder_out, value=bert_encoder_out, key_padding_mask=bert_encoder_padding_mask)
        x2 = F.dropout(x2, p=self.dropout, training=self.training)
        x2 = residual + x2
        x2 = self.maybe_layer_norm(self.self_attn_layer_norm, x2, after=True)#c
        
        x3 = residual + (0.5*x) + (0.5*x2)

        residual = x3
        x3 = self.maybe_layer_norm(self.final_layer_norm, x3, before=True) # b
        x3 = self.activation_fn(self.fc1(x3))
        x3 = F.dropout(x3, p=self.activation_dropout, training=self.training)
        x3 = self.fc2(x3)
        x3 = F.dropout(x3, p=self.dropout, training=self.training)

        #ratios = self.get_ratio()
        #x = residual + ratios[0] * x + ratios[1] * x2 ####
        x3 = residual + x3
        
        x3 = self.maybe_layer_norm(self.final_layer_norm, x3, after=True) #b
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            residual = x
            x1 = self.maybe_layer_norm(self.encoder_attn_layer_norm, x1, before=True) #2
            ...
            x1 = F.dropout(x1, p=self.dropout, training=self.training)
            x1 = residual + x1
            x1 = self.maybe_layer_norm(self.encoder_attn_layer_norm, x1, after=True) # 2

            x2 = self.maybe_layer_norm(self.encoder_attn_layer_norm, x2, before=True)#4
            x2, _ = self.bert_attn(
                query=x2,
            ...
            x2 = F.dropout(x2, p=self.dropout, training=self.training)
            x2 = residual + x2
            x2 = self.maybe_layer_norm(self.encoder_attn_layer_norm, x2, after=True) # 4

            x = residual + (0.5 * x1) + (0.5 * x2)

        residual = x
        x = self.maybe_layer_norm(self.final_layer_norm, x, before=True) # 3
        x = self.activation_fn(self.fc1(x))
        x = F.dropout(x, p=self.activation_dropout, training=self.training)
        x = self.fc2(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        x = self.maybe_layer_norm(self.final_layer_norm, x, after=True) # 3
